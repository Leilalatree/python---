...
#     'Cookie':'JSESSIONID=BE  3B5FCADF59CADC186FD8F170899B3E; route=c5c62a339e7744272a54643b3be5bf64; BIGipServerotn=2129133834.24610.0000; BIGipServerpassport=988283146.50215.0000; _jc_save_showIns=true; current_captcha_type=Z; _jc_save_fromStation=%u6D1B%u9633%2CLLF; _jc_save_toStation=%u4E0A%u6D77%2CAOH; _jc_save_fromDate=2017-10-11; _jc_save_toDate=2017-10-04; _jc_save_wfdc_flag=dc'
# }

ssl._create_default_https_context = ssl._create_unverified_context

c = cookielib.LWPCookieJar()#生成储存cookie的对象
...
def getCode():
    req = urllib2.Request(
        'https://kyfw.12306.cn/passport/captcha/captcha-image?login_site=E&module=login&rand=sjrand&0.12173702242612605')
    req.headers = headers
    code_img = opener.open(req).read()
...
def code():
    ...
    html = res.read()  # 拿到网页源代码
    result = loads(html)  # 把json字符串转换成字典
...
def login():
    ...
    req.headers = headers
    data = {
        'username': user12306.user,
        'password':user12306.pwd,
        'appid': 'otn'
    }
...
def getTrain():
    ...
    html = opener.open(req).read()
    print 118,html
    req = urllib2.Request('https://kyfw.12306.cn/otn/leftTicket/queryX?leftTicketDTO.train_date=%s&leftTicketDTO.from_station=%s&leftTicketDTO.to_station=%s&purpose_codes=ADULT' %(
        train_date, from_station, to_station))
    req.headers = headers
    res = opener.open(req)
    html = res.read()
    print 119,res.geturl()
    try:
        result = loads(html)
...
def order(secretStr,tmp_list):
    ...
    #     'back_train_date': '2017-10-03',  #下单日期
    #     'tour_flag': 'dc',
    #     'purpose_codes': 'ADULT',
    #     'query_from_station_name': '洛阳',
    #     'query_to_station_name': '上海',
    #     'undefined': ''
    # }
    data = 'secretStr={}&train_date={}&back_train_date={}&tour_flag=dc&purpose_codes=ADULT&query_from_station_name=洛阳&query_to_station_name=上海&undefined'.format(
        secretStr, train_date, time.strftime('%Y-%m-%d'))
    print 11111, data
    # data = urllib.urlencode(data)
    req.headers = headers
...
'passengerTicketStr':'O,0,1,黄日新,1,142202199601010114,15921052260,N',
'oldPassengerStr':'黄日新,1,142202199601010114,3_',
'tour_flag':'dc',
'randCode':'',
'_json_att':'',
    'REPEAT_SUBMIT_TOKEN':REPEAT_SUBMIT_TOKEN
    ...
    req.headers = headers
    data = {
        'train_date': 'Wed Oct 11 2017 00:00:00GMT + 0800(中国标准时间)',
