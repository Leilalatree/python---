...
class WXRobot(object):
    ...
    def save_firends_into_mysql(self):
        print('保存好友到数据库')
        sql = 'insert into friend(username, nickname, sex) values(%s, %s, %s)'
        friends_list = [[f['UserName'], f['NickName'], f['Sex']] for f in self.member_list]
        insert_many(sql, friends_list)
    ...
    '''
       ...
       第一个synccheck的参数 synckey是从 init 的response获取
       只要访问了webwxsyn，就会更新synckey
   '''
...
CREATE TABLE `friend` (
  ...
  `UserName` char(80) COLLATE utf8_bin DEFAULT NULL,
  `NickName` char(80) COLLATE utf8_bin DEFAULT NULL,
